...
class MaterializeNotificationFromTaskArchivedUseCaseTest {

    private val notificationService: NotificationService = mockk()
    private val useCase = MaterializeNotificationFromTaskArchivedUseCase(notificationService = notificationService)

    @Test
    fun `should materialize a TASK_ARCHIVED notification`() {
        // given
        ...
        // when
        val result = useCase.execute(taskId = taskId, userId = userId, archivedAt = Instant.now())

        // then
        assertEquals(taskId, result.taskId)
        assertEquals(userId, result.userId)
        assertEquals(NotificationType.TASK_ARCHIVED, result.type)
