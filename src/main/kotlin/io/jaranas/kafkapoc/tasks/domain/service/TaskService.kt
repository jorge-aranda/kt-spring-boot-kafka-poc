...
class TaskService(
    ...
    fun findByIdForUser(taskId: String, userId: String): Task {
    ...
    fun listActiveForUser(userId: String): List<Task> =
        taskRepository.findByUserIdAndArchivedFalse(userId = userId)

    fun complete(taskId: String, userId: String): Task {
    ...
    fun archive(taskId: String, userId: String): Task {
        val task = findByIdForUser(taskId = taskId, userId = userId)
